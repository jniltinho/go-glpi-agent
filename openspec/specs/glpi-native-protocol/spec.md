# glpi-native-protocol Specification

## Purpose

Protocolo primário para `--server` contra GLPI 10+: agent ID UUID, detecção de
servidor nativo vs legado, requisição CONTACT, serialização de inventário em JSON
e os headers/compressão do protocolo nativo.
