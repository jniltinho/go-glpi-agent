# windows-packaging Specification

## Purpose

Produce and validate a Windows distribution — a static `.exe` packaged as a `.zip` with a
PowerShell installer that registers a Scheduled Task — built on the existing Linux release
runner and validated end-to-end against a GLPI 10 stack via Vagrant.
